...
if (template.indexOf("!") === -1) {
  // FIX: Properly handle paths with spaces by using path.normalize
  const loaderPath = path.normalize(require.resolve("./lib/loader.js"));
  const templatePath = path.resolve(context, template);
  template = loaderPath + "!" + templatePath;
}
...
return template.replace(
  ...
  (match, prefix, filepath, postfix) => {
    // FIX: Normalize resolved paths to handle spaces properly
    const resolvedPath = path.resolve(filepath);
    return prefix + path.normalize(resolvedPath) + postfix;
  }
);
  }
